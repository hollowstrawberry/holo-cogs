...
Can interface with other cogs like [bz-cogs](https://github.com/zhaobenny/bz-cogs)'s aimage and [crab-cogs](https://github.com/hollowstrawberry/crab-cogs)'s imagescanner to generate and revise Stable Diffusion images using natural conversation. Now uses my own aimage cog, which is very specific to my case, so might not be useful for you anymore.

⚠️ This cog is **not meant for public use**, you are using it at your own risk. There are **no safeguards against abuse**. It uses openai or openrouter models, and the approach to memory means it will use up to 3 times as many input tokens as you would expect, so you may face large monetary charges (I pay around 20 dollars a month after frequent use from many users). It does however let you customize the different limits of tokens used by the bot. It also optionally stores text-based \"memories\" which may contain information about users. Some of these memories as well as the recent chat logs are sent to openai/openrouter servers every time the bot user is pinged. As the bot's owner you become wholly responsible for the data of your users and the money that user interactions will consume.
