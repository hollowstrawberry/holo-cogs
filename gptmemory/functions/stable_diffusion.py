...
class StableDiffusionFunctionCall(FunctionCallBase):
    ...
    async def run(self, arguments: dict) -> str:
        ...
        if existing and not message:
            response = f"[Image generation started, but the original image was not found, so it cannot be revised and a new one will be made from scratch. Notify the user.]"
        elif existing and not sent_by_me:
            response = f"[Image generation started, but the original image was not sent by the bot, so it cannot be revised and a new one will be made from scratch. Notify the user.]"
        else:
            response = f"[Image generation started successfully]"
        
        if existing and aspect_ratio:
